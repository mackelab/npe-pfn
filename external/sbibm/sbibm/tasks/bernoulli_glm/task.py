...
class BernoulliGLM(Task):
    ...
    def get_simulator(self, max_calls: Optional[int] = None) -> Simulator:
        ...
        stimulus_I = torch.load(self.path / "files" / "stimulus_I.pt", weights_only=False).to(device)
        design_matrix = torch.load(self.path / "files" / "design_matrix.pt", weights_only=False).to(device)
    ...
    def _sample_reference_posterior(
        ...
        design_matrix = torch.load(self.path / "files" / "design_matrix.pt", weights_only=False)
