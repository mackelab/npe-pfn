    ...
    def get_all_preprocessors(
        ...
        all_preprocessors = {
            ...
            "quantile_uni_coarse": QuantileTransformer(
                output_distribution="uniform",
                n_quantiles=min(max(num_examples // 10, 2),9999),
                random_state=random_state,
            ),
            ...
            "quantile_uni": QuantileTransformer(
                output_distribution="uniform",
                n_quantiles=min(max(num_examples // 5, 2), 9999), # NOTE: This can not be larger than subsample which is 10_000 by default in Sklearn
                random_state=random_state,
            ),
            "quantile_norm": QuantileTransformer(
                output_distribution="normal",
                n_quantiles=min(max(num_examples // 5, 2),9999),
                random_state=random_state,
            ),
            "quantile_uni_fine": QuantileTransformer(
                output_distribution="uniform",
                n_quantiles=min(num_examples,9999),
                random_state=random_state,
            ),
