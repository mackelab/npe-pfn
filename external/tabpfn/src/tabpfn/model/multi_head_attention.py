...
_flex_attention = torch.compile(
    flex_attention, options={"triton.cudagraphs": True}, fullgraph=True
)
...
@contextmanager
def use_flex_attention(enable: bool):
    global USE_FLEX_ATTENTION
    original_state = USE_FLEX_ATTENTION
    USE_FLEX_ATTENTION = enable
    try:
        yield
    finally:
        USE_FLEX_ATTENTION = original_state


def set_flex_attention(enable: bool):
    global USE_FLEX_ATTENTION
    USE_FLEX_ATTENTION = enable
...
class MultiHeadAttention(torch.nn.Module):
    ...
    def compute_attention_heads(  # noqa: C901, PLR0912
        ...
        if USE_FLEX_ATTENTION:
            ...
            # Only if sequence length is large enough, we use flex_attention
            # NOTE: TORCH compile will break if there are two different attentions with
            # different input shapes i.e. features vs train_set
            # We thus simply use the compiled flex_attention for all cases where the eager
            # attention fails...
            # NOTE: This will break when feature_dim > 20_000 due to bug in torch dynamo
            if seqlen_q > 1_000:
                out = _flex_attention(
                    q.transpose(1, 2),
                    k.transpose(1, 2),
            ...
            attention_head_outputs = out.transpose(1, 2)
