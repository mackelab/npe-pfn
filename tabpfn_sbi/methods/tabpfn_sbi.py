...
from tabpfn_sbi.methods.sampler import accept_reject_sample
from tabpfn_sbi.methods.sbi_baselines import get_embedding_net
from tabpfn_sbi.methods.tabpfn_support_posterior import get_filtering_method
...
def run_basic_tabpfn_interface(cfg, task):
    """Run basic TabPFN interface with support for filtering.

    Args:
        cfg: Configuration object containing method and task settings
        task: Task object providing prior and simulator
    """
    ...
    if cfg.method.name == "tabpfn":
        ...
        cfg.method.name == "filtered_tabpfn"
        or cfg.method.name == "filtered_tabpfn_binary"
    ):
        method = FilteredTabPFNSBI
        ...
        method_kwargs = {
            ...
            "regressor_init_kwargs": cfg.method.regressor_init_kwargs,
            "classifier_init_kwargs": cfg.method.classifier_init_kwargs,
            "filter_type": cfg.method.filter_type,
            "filter_context_size": cfg.method.filter_context_size,
        }
    elif cfg.method.name == "uncond_tabpfn":
        method = UnconditionalTabPFNSBI

        method_kwargs = {
            "num_clusters": cfg.method.num_clusters,
            "show_progress_bars": cfg.method.show_progress_bars,
            "regressor_init_kwargs": cfg.method.regressor_init_kwargs,
            "classifier_init_kwargs": cfg.method.classifier_init_kwargs,
        }
    elif (
        cfg.method.name == "tabpfn_infomax"
        or cfg.method.name == "filtered_tabpfn_infomax"
    ):
        input_shape = xs.shape[1:]
        embedding_net = get_embedding_net(
            cfg.method.embedding_net.name,
            input_shape,
            **cfg.method.embedding_net.params,
        )
        # If pretraining is enabled, train the embedding net
        if hasattr(cfg.method, "pretraining"):
            embedding_net = run_pretraining(cfg, task, embedding_net, thetas, xs)
        else:
            raise ValueError(
                "Pretraining is required for TabPFN Infomax, but not enabled."
            )

        method = TabPFNSBI if cfg.method.name == "tabpfn_infomax" else FilteredTabPFNSBI
        method_kwargs = {
            "prior": task.get_prior_dist(),
            "show_progress_bars": cfg.method.show_progress_bars,
            "regressor_init_kwargs": cfg.method.regressor_init_kwargs,
        ...
        raise ValueError(f"Unknown TabPFN method: {cfg.method.name}")

    posterior = method(**method_kwargs)

    posterior.append_simulations(thetas, xs)

    posterior.sample = partial(
        posterior.sample, max_sampling_batch_size=cfg.method.max_sampling_batch_size
    )
...
class FilteredTabPFNSBI(TabPFNSBI):
    def __init__(
        self,
        show_progress_bars: bool = False,
        prior: Optional[Distribution] = None,
        filter_type: (
            Literal[
                ...
                "binary_hamming_filtering",
                "binary_jaccard_filtering",
                "binary_dice_filtering",
            ]
            | callable
        ) = "standardized_euclidean_filtering",
...
class UnconditionalTabPFNSBI(TabPFNSBI):
    # will essentially just be a generative model of the prior
    ...
    # NOTE: Does not support batched sampling at the moment as this is normally handled by the accept_reject_sample
    def sample(
        self,
