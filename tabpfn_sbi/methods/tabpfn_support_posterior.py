...
def get_filtering_method(name: str):
    ...
    elif name == "binary_hamming_filtering":
        return binary_hamming_filtering
    elif name == "binary_jaccard_filtering":
        return binary_jaccard_filtering
    elif name == "binary_dice_filtering":
        return binary_dice_filtering
    elif callable(name):
        return name
    else:
...
def binary_hamming_filtering(obs: Tensor, theta: Tensor, x: Tensor, context_size: int):
    # For binary vectors, compute Hamming distance (number of differing bits)
    # Hamming distance = sum of absolute differences between binary vectors
    dists = torch.sum(torch.abs(x - obs), dim=1)

    _, idx = torch.topk(dists, min(context_size, dists.shape[0]), largest=False)
    return theta[idx], x[idx]


def binary_jaccard_filtering(obs: Tensor, theta: Tensor, x: Tensor, context_size: int):
    # For binary vectors, compute Jaccard distance
    # Jaccard distance = 1 - (intersection / union)
    # intersection = sum of min(x_i, obs_i) for each dimension
    # union = sum of max(x_i, obs_i) for each dimension

    # Compute intersection (element-wise minimum)
    intersection = torch.sum(torch.minimum(x, obs), dim=1)

    # Compute union (element-wise maximum)
    union = torch.sum(torch.maximum(x, obs), dim=1)

    # Avoid division by zero
    union = torch.where(union == 0, 1.0, union)

    # Jaccard similarity = intersection / union
    jaccard_similarity = intersection / union

    # Jaccard distance = 1 - similarity
    dists = 1 - jaccard_similarity

    _, idx = torch.topk(dists, min(context_size, dists.shape[0]), largest=False)
    return theta[idx], x[idx]


def binary_dice_filtering(obs: Tensor, theta: Tensor, x: Tensor, context_size: int):
    # For binary vectors, compute Dice distance
    # Dice distance = 1 - (2 * intersection / (sum_x + sum_obs))
    # intersection = sum of min(x_i, obs_i) for each dimension

    # Compute intersection (element-wise minimum)
    intersection = torch.sum(torch.minimum(x, obs), dim=1)

    # Compute sum of each vector
    sum_x = torch.sum(x, dim=1)
    sum_obs = torch.sum(obs, dim=1)

    # Compute denominator: sum_x + sum_obs
    denominator = sum_x + sum_obs

    # Avoid division by zero
    denominator = torch.where(denominator == 0, 1.0, denominator)

    # Dice similarity = 2 * intersection / (sum_x + sum_obs)
    dice_similarity = 2 * intersection / denominator

    # Dice distance = 1 - similarity
    dists = 1 - dice_similarity
