...
class streampepperdf(streamdf):
    ...
    def __init__(self, *args, **kwargs):
        ...
        timpact = kwargs.pop("timpact", [1.0])
        impactb = kwargs.pop("impactb", None)
        if impactb is None:
            sim_setup = True  # we're just getting ready to run sims
            impactb = [0.0 for t in timpact]
        else:
            sim_setup = False
        subhalovel = kwargs.pop(
            "subhalovel", numpy.array([[0.0, 1.0, 0.0] for t in timpact])
        )
        impact_angle = kwargs.pop(
            "impact_angle", numpy.array([0.0001 for t in timpact])
        )
        ...
        if GM is None and rs is None and subhalopot is None:
            ...
            GM = [
                10**-5.0 / bovy_conversion.mass_in_1010msol(220.0, 8.0) for t in timpact
            ]
            rs = [0.04 / 8.0 for t in timpact]
        if GM is None:
            GM = [None for b in impactb]
        if rs is None:
            rs = [None for b in impactb]
        if subhalopot is None:
            subhalopot = [None for b in impactb]
        ...
        length_factor = kwargs.pop("length_factor", 1.0)
        ...
        # Adjust the angles
        if not self._leading and sim_setup:
            impact_angle *= -1.0
        ...
        for ti in self._uniq_timpact:
            ...
            if not self._leading:
                sgapdf_kwargs["impact_angle"] = -1.0
            ...
            self._sgapdfs_coordtransform[ti] = streamgapdf(*args, **sgapdf_kwargs)
        # Also setup coordtransform for the current time, for transforming
        if not 0.0 in self._uniq_timpact:
            ti = 0.0
            sgapdf_kwargs = copy.deepcopy(kwargs)
            sgapdf_kwargs["timpact"] = ti
            sgapdf_kwargs["impact_angle"] = impact_angle[0]  # only affects a check
            ...
            sgapdf_kwargs["spline_order"] = spline_order
            sgapdf_kwargs["hernquist"] = hernquist
            sgapdf_kwargs["GM"] = 0.0  # Just to avoid error
            sgapdf_kwargs["rs"] = rs[0]
            sgapdf_kwargs["subhalopot"] = subhalopot[0]
            sgapdf_kwargs["nokicksetup"] = True
            self._sgapdfs_coordtransform[ti] = streamgapdf(*args, **sgapdf_kwargs)
            ...
            self._sgapdfs_coordtransform[ti]._impact_angle = 0.0
            self._sgapdfs_coordtransform[ti]._interpolate_stream_track_kick()
            self._sgapdfs_coordtransform[ti]._interpolate_stream_track_kick_aA()
            self._interpolatedObsTrack = self._sgapdfs_coordtransform[
                ti
            ]._kick_interpolatedObsTrack
            self._ObsTrack = self._sgapdfs_coordtransform[ti]._gap_ObsTrack
            self._interpolatedObsTrackXY = self._sgapdfs_coordtransform[
                ti
            ]._kick_interpolatedObsTrackXY
            self._ObsTrackXY = self._sgapdfs_coordtransform[ti]._gap_ObsTrackXY
            self._alljacsTrack = self._sgapdfs_coordtransform[ti]._gap_alljacsTrack
            self._allinvjacsTrack = self._sgapdfs_coordtransform[
                ti
            ]._gap_allinvjacsTrack
            self._interpolatedObsTrackAA = self._sgapdfs_coordtransform[
                ti
            ]._kick_interpolatedObsTrackAA
            self._ObsTrackAA = self._sgapdfs_coordtransform[ti]._gap_ObsTrackAA
            self._interpolatedThetasTrack = self._sgapdfs_coordtransform[
                ti
            ]._kick_interpolatedThetasTrack
            self._thetasTrack = self._sgapdfs_coordtransform[ti]._gap_thetasTrack
            self._nTrackChunks = self._sgapdfs_coordtransform[ti]._nTrackChunksImpact
        self._gap_leading = self._sgapdfs_coordtransform[timpact[0]]._gap_leading
        # Compute all kicks
        self._spline_order = spline_order
        self.hernquist = hernquist
        self._determine_deltaOmegaTheta_kicks(
            impact_angle, impactb, subhalovel, timpact, GM, rs, subhalopot
        )
    ...
    def _setup_timeAndAngleSampling(self, length_factor):
        ...
        for ti in self._uniq_timpact:
            ...
            max_apar = self.length(tdisrupt=self._tdisrupt - ti) * self._length_factor
            ...
            dXda = self._sgapdfs_coordtransform[ti]._kick_interpTrackX.derivative()
            dYda = self._sgapdfs_coordtransform[ti]._kick_interpTrackY.derivative()
            dZda = self._sgapdfs_coordtransform[ti]._kick_interpTrackZ.derivative()
            apars = numpy.linspace(
                0.0, max_apar, self._sgapdfs_coordtransform[ti]._nKickPoints
            )
            cumul_stream_len = numpy.hstack(
                (
                    0.0,
                    numpy.array(
                        [
                            ...
                            for al, au in zip(apars[:-1], apars[1:])
                        ]
                    ),
                )
            )
            ...
            self._icdf_stream_len[ti] = interpolate.InterpolatedUnivariateSpline(
                cumul_stream_len / cumul_stream_len[-1], apars, k=3
            )
        # Now determine relative probability of different times
        self._ptimpact = numpy.array(
            [self._stream_len[ti] for ti in self._uniq_timpact]
        )
    ...
    def simulate(
        ...
        timpacts = numpy.array(self._uniq_timpact)[
            numpy.random.choice(len(self._uniq_timpact), size=nimpact, p=self._ptimpact)
        ]
        # Sample angles from the part of the stream that existed then
        impact_angles = numpy.array(
            [self._icdf_stream_len[ti](numpy.random.uniform()) for ti in timpacts]
        )
        ...
        # impact b
        impactbs = (
            (2.0 * numpy.random.uniform(size=len(impact_angles)) - 1.0) * Xrs * rss
        )
        # velocity
        subhalovels = numpy.empty((len(impact_angles), 3))
        for ii in range(len(timpacts)):
            subhalovels[ii] = self._draw_impact_velocities(
                timpacts[ii], sigma, impact_angles[ii], n=1
            )[0]
        # Flip angle sign if necessary
        if not self._gap_leading:
            impact_angles *= -1.0
        # Setup
        self.set_impacts(
    ...
    def _draw_impact_velocities(self, timpact, sigma, impact_angle, n=1):
        ...
        # build vx and vy
        theta = numpy.random.uniform(size=n) * 2.0 * numpy.pi
        ...
        self._sgapdfs_coordtransform[timpact]._interpolate_stream_track_kick()
        streamDir = numpy.array(
            [
                self._sgapdfs_coordtransform[timpact]._kick_interpTrackvX(impact_angle),
                self._sgapdfs_coordtransform[timpact]._kick_interpTrackvY(impact_angle),
                self._sgapdfs_coordtransform[timpact]._kick_interpTrackvZ(impact_angle),
            ]
        )
        # Build the rotation matrices and their inverse
        rotinv = _rotation_vy(streamDir[:, numpy.newaxis].T, inv=True)[0]
        out = numpy.sum(
    ...
    def set_impacts(self, **kwargs):
        ...
        """
        # Parse kwargs, everything except for timpact can be arrays
        impactb = kwargs.pop("impactb", [1.0])
        subhalovel = kwargs.pop("subhalovel", numpy.array([[0.0, 1.0, 0.0]]))
        impact_angle = kwargs.pop("impact_angle", [1.0])
        GM = kwargs.pop("GM", None)
        rs = kwargs.pop("rs", None)
        subhalopot = kwargs.pop("subhalopot", None)
        if GM is None:
    ...
    def _determine_deltaOmegaTheta_kicks(
        self, impact_angle, impactb, subhalovel, timpact, GM, rs, subhalopot
    ):
    ...
    def approx_kicks(self, threshold, relative=True):
        ...
        for ii, ti in enumerate(self._uniq_timpact_sim):
            ...
            keepIndx = numpy.zeros(
                len(self._sgapdfs_uniq[ii]._kick_interpdOpar_poly.c[-1]), dtype="bool"
            )
            ...
            for kk in kickIndx:
                ...
                # find peaks
                relKick = numpy.fabs(self._sgapdfs[kk]._kick_interpdOpar(tx))
                peakLeftIndx = numpy.argmax(
                    relKick
                    / numpy.amax(relKick)
                    * (tx <= self._sgapdfs[kk]._impact_angle)
                )
                peakRightIndx = numpy.argmax(
                    relKick
                    / numpy.amax(relKick)
                    * (tx > self._sgapdfs[kk]._impact_angle)
                )
            ...
            self._sgapdfs_uniq[ii]._kick_interpdOpar_poly.c[-1, True - keepIndx] = 0.0
            self._sgapdfs_uniq[ii]._kick_interpdOpar_poly.c[-2, True - keepIndx] = 0.0
            c12 = (
                self._sgapdfs_uniq[ii]._kick_interpdOpar_poly.c[-1]
                * self._sgapdfs_uniq[ii]._kick_interpdOpar_poly.c[-2]
            )
            nzIndx = numpy.nonzero(
                (c12 != 0.0) + (numpy.roll(c12, 1) - c12 != 0.0) + (range_tx == 0)
            )[0]
            nzIndx = numpy.nonzero(
                (c12 != 0.0)
                + (numpy.roll(c12, 1) - c12 != 0.0)
                + (numpy.roll(c12, 2) - c12 != 0.0)
                + (c12 - numpy.roll(c12, -1) != 0.0)
                + (range_tx == 0)
            )[0]
            # Linearly interpolate over dropped ranges
            droppedIndx = numpy.nonzero(
                ((c12 == 0.0) * (numpy.roll(c12, 1) - c12 != 0.0))
                + ((c12 == 0.0) * (range_tx == 0))
            )[0]
            for dd in droppedIndx:
                if dd == 0:
                    ...
                    self._sgapdfs_uniq[ii]._kick_interpdOpar_poly.c[-1, dd] = (
                        self._sgapdfs_uniq[ii]._kick_interpdOpar_poly.c[-1, dd - 1]
                        + (
                            self._sgapdfs_uniq[ii]._kick_interpdOpar_poly.x[dd]
                            - self._sgapdfs_uniq[ii]._kick_interpdOpar_poly.x[dd - 1]
                        )
                        * self._sgapdfs_uniq[ii]._kick_interpdOpar_poly.c[-2, dd - 1]
                    )
                ...
                if nextIndx == len(nzIndx) - 1:
                    ...
                    nextx = self._sgapdfs_uniq[ii]._kick_interpdOpar_poly.x[nextIndx]
                    nextVal = self._sgapdfs_uniq[ii]._kick_interpdOpar_poly.c[
                        -1, nextIndx
                    ]
            ...
            self._sgapdfs_uniq[ii]._kick_interpdOpar_poly = interpolate.PPoly(
                ...
                self._sgapdfs_uniq[ii]._kick_interpdOpar_poly.x[
                    numpy.hstack((nzIndx, [len(tx)]))
                ],
            )
        return None
    ...
    def pOparapar(self, Opar, apar):
        ...
        for kk, timpact in enumerate(self._timpact):
            ...
            afterIndx = (
                remaining_indx * (ts < (timpact - current_timpact)) * (ts >= 0.0)
            )
            out[afterIndx] = super(streampepperdf, self).pOparapar(
                current_Opar[afterIndx],
        ...
        if numpy.sum(remaining_indx) > 0:
            # Evaluate
            out[remaining_indx] = super(streampepperdf, self).pOparapar(
                current_Opar[remaining_indx],
                current_apar[remaining_indx],
                tdisrupt=self._tdisrupt - current_timpact,
            )
        return out

    def _density_par(
        self, dangle, tdisrupt=None, approx=True, force_indiv_impacts=False
    ):
        ...
        if len(self._timpact) == 0:
            return super(streampepperdf, self)._density_par(dangle, tdisrupt=tdisrupt)
        if tdisrupt is None:
            tdisrupt = self._tdisrupt
        if approx:
            return self._density_par_approx(dangle, tdisrupt, force_indiv_impacts)
        else:
    ...
    def _densityAndOmega_par_approx(self, dangle):
        ...
        return (
            self._density_par_approx(
                dangle, self._tdisrupt, False, False, ul, da, ti, c0, c1
            ),
            self._meanOmega_approx(dangle, self._tdisrupt, False, ul, da, ti, c0, c1),
        )

    def _density_par_approx(
        self, dangle, tdisrupt, force_indiv_impacts, _return_array=False, *args
    ):
        ...
        lowbindx, lowx, edge = self.minOpar(
            dangle, False, force_indiv_impacts, True, ul, da, ti, c0, c1
        )
        ...
        out = (
            ...
            * (
                ...
                - special.erf(
                    1.0
                    / numpy.sqrt(2.0 * self._sortedSigOEig[2])
                    * (ul - c0 - self._meandO - c1 * (ul - numpy.roll(ul, 1)))
                )
            )
        )
        if _return_array:
        ...
        out += 0.5 * (
            1.0
            + special.erf(
                (self._meandO - ul[-1]) / numpy.sqrt(2.0 * self._sortedSigOEig[2])
            )
        )
    ...
    def _approx_pdf(self, dangle, force_indiv_impacts=False):
        ...
        Oparb = (dangle - sgapdfs[0]._kick_interpdOpar_poly.x) / timpact[0]
        ul = Oparb[::-1]
        # Array of previous pw-poly coeffs and breaks
        pwpolyBreak = sgapdfs[0]._kick_interpdOpar_poly.x[::-1]
        pwpolyCoeff0 = numpy.append(sgapdfs[0]._kick_interpdOpar_poly.c[-1], 0.0)[::-1]
        pwpolyCoeff1 = numpy.append(sgapdfs[0]._kick_interpdOpar_poly.c[-2], 0.0)[::-1]
        ...
        c0 = pwpolyCoeff0
        c1 = 1.0 + pwpolyCoeff1 * timpact[0]
        cx = numpy.zeros(len(ul))
        for kk in range(1, len(timpact)):
            ul, da, ti, do, c0, c1, cx, pwpolyBreak, pwpolyCoeff0, pwpolyCoeff1 = (
                self._update_approx_prevImpact(
                    kk,
                    ul,
                    da,
                    ti,
    ...
    def _update_approx_prevImpact(
        ...
        xj = numpy.tile(sgapdfs[kk]._kick_interpdOpar_poly.x[::-1], (len(ul), 1)).T
        ult = (da_u - xj) / ti_u
        ...
        da_u = numpy.append(
            numpy.tile(da_u, (nNewCoeff, 1))[limitIndx].flatten(), da_u
        )[limitsIndx]
        ti_u = numpy.append(
            numpy.tile(ti_u, (nNewCoeff, 1))[limitIndx].flatten(), ti_u
        )[limitsIndx]
        do_u = numpy.append(numpy.tile(do, (nNewCoeff, 1))[limitIndx].flatten(), do)[
            limitsIndx
        ]
        c0_u = numpy.append(
            numpy.tile(c0_u, (nNewCoeff, 1))[limitIndx].flatten(), c0_u
        )[limitsIndx]
        c1_u = numpy.append(numpy.tile(c1, (nNewCoeff, 1))[limitIndx].flatten(), c1)[
            limitsIndx
        ]
        cx_u = numpy.append(
            numpy.tile(cx_u, (nNewCoeff, 1))[limitIndx].flatten(), cx_u
        )[limitsIndx]
        # Also update the previous coefficients, figuring out where old limits
        # were inserted
        insertIndx = numpy.sort(
            limitusIndx[numpy.arange(numpy.sum(limitIndx), len(ul_u))]
        )[:-1]
        pwpolyBreak_u = numpy.append(xj[limitIndx].flatten(), numpy.zeros(len(ul)))[
            limitsIndx
        ]
        pwpolyCoeff0_u = numpy.append(
            numpy.tile(
                numpy.append(sgapdfs[kk]._kick_interpdOpar_poly.c[-1], 0.0)[::-1],
                (len(ul), 1),
            )
            .T[limitIndx]
            .flatten(),
        ...
        pwpolyCoeff1_u = numpy.append(
            numpy.tile(
                numpy.append(sgapdfs[kk]._kick_interpdOpar_poly.c[-2], 0.0)[::-1],
                (len(ul), 1),
            )
            .T[limitIndx]
            .flatten(),
        ...
        return (
            ...
            c1_u[dupIndx],
            cx_u[dupIndx],
            pwpolyBreak_u[dupIndx],
            pwpolyCoeff0_u[dupIndx],
    ...
    def minOpar(
        ...
        *args,
        ...
        nlowx[0] *= -1.0  # need to adjust sign bc of roll
        lowbindx = numpy.argmax((lowx >= 0.0) * (nlowx <= 10.0))  # not too crazy
    ...
    def _minOpar_bruteforce(self, dangle):
        ...
        sig = numpy.array(
            [numpy.sqrt(self._sortedSigOEig[2]), numpy.sqrt(self._sortedSigOEig[2])]
        )
        while numpy.all(self.pOparapar(nzguess, dangle) == 0.0):
            nzguess += sig / 3.0
        nzguess = nzguess[nzguess != 0.0][0]
        nzval = self.pOparapar(nzguess, dangle)
        guesso = nzguess - numpy.sqrt(self._sortedSigOEig[2])
        while self.pOparapar(guesso, dangle) - 10.0**-6.0 * nzval > 0.0:
            guesso -= numpy.sqrt(self._sortedSigOEig[2])
        return (
            optimize.brentq(
                lambda x: self.pOparapar(x * nzguess, dangle) - 10.0**-6.0 * nzval,
                guesso / nzguess,
                1.0,
                xtol=1e-8,
                rtol=1e-8,
    ...
    def meanOmega(
        ...
        """
        if len(self._timpact) == 0:
            out = super(streampepperdf, self).meanOmega(
                dangle, tdisrupt=tdisrupt, oned=oned
            )
            if not norm:
                return out * super(streampepperdf, self)._density_par(
                    dangle, tdisrupt=tdisrupt
                )
            else:
                return out
        if tdisrupt is None:
    ...
    def _meanOmega_approx(self, dangle, tdisrupt, force_indiv_impacts, *args):
        ...
        dens_arr = self._density_par_approx(
            dangle, tdisrupt, force_indiv_impacts, True, ul, da, ti, c0, c1
        )
        dens = self._density_par_approx(
            dangle, tdisrupt, force_indiv_impacts, False, ul, da, ti, c0, c1
        )
        ...
        out = numpy.sum(
            (
                (ul + (self._meandO + c0 - ul) / c1) * dens_arr
                + numpy.sqrt(self._sortedSigOEig[2] / 2.0 / numpy.pi)
                / c1**2.0
                * (
                    numpy.exp(
                        -0.5
                        * (ul - c0 - c1 * (ul - numpy.roll(ul, 1)) - self._meandO)
                        ** 2.0
                        / self._sortedSigOEig[2]
                    )
                    - numpy.exp(
                        -0.5 * (ul - c0 - self._meandO) ** 2.0 / self._sortedSigOEig[2]
                    )
                )
            )[lowbindx:]
        )
        ...
        out += 0.5 * (
            ...
            * numpy.sqrt(self._sortedSigOEig[2])
            * numpy.exp(-0.5 * (self._meandO - ul[-1]) ** 2.0 / self._sortedSigOEig[2])
            + self._meandO
            * (
                1.0
                + special.erf(
        ...
        if edge:
            out += 0.5 * (
                ...
                * (
                ...
                + numpy.sqrt(2.0 * self._sortedSigOEig[2] / numpy.pi)
                * (
                    numpy.exp(
                        -0.5
                        * (dangle / tdisrupt - self._meandO) ** 2.0
                        / self._sortedSigOEig[2]
                    )
                    - numpy.exp(
                        -0.5 * (ul[0] - self._meandO) ** 2.0 / self._sortedSigOEig[2]
                    )
                )
            )
        return out / dens
    ...
    def meanTrack(
        self, dangle, approx=True, force_indiv_impacts=False, coord="XY", _mO=None
    ):
        """
        NAME:
        ...
        mO = (
            numpy.tile(self._progenitor_Omega, (_mO.shape[0], 1)).T
            + _mO
            * numpy.tile(self._dsigomeanProgDirection, (_mO.shape[0], 1)).T
            * self._sigMeanSign
        )
        da = (
            numpy.tile(self._progenitor_angle, (_mO.shape[0], 1)).T
            + dangle
            * numpy.tile(self._dsigomeanProgDirection, (_mO.shape[0], 1)).T
            * self._sigMeanSign
        )
        ...
        XY[2] = out[3]
        TrackvX, TrackvY, TrackvZ = bovy_coords.cyl_to_rect_vec(
            out[1], out[2], out[4], out[5]
        )
        XY[3] = TrackvX
        XY[4] = TrackvY
        XY[5] = TrackvZ
        ...
        vXYZ = bovy_coords.galcenrect_to_vxvyvz(
            XY[3] * self._vo,
            XY[4] * self._vo,
            XY[5] * self._vo,
            vsun=self._vsun,
        ...
        vlbd = bovy_coords.vxvyvz_to_vrpmllpmbb(
            vXYZ[0], vXYZ[1], vXYZ[2], lbd[:, 0], lbd[:, 1], lbd[:, 2], degree=True
        )
        if coord.lower() == "lb":
    ...
    def _structure_hash(self, digit, apars):
        return hashlib.md5(
            numpy.hstack(([digit], apars, self._impact_angle))
        ).hexdigest()

    def csd(self, d1="density", d2="density", apars=None):
        if d1.lower() == "density" or d2.lower() == "density":
            ...
            if hasattr(self, "_dens_hash") and new_hash == self._dens_hash:
                ...
                dens_unp = numpy.array(
                    [
                        super(
                            galpy.df.streampepperdf.streampepperdf, self
                        )._density_par(a)
                        for a in apars
                    ]
                )
        ...
        if d1.lower() == "meanomega" or d2.lower() == "meanomega":
            ...
            if hasattr(self, "_mO_hash") and new_hash == self._mO_hash:
                ...
                mO = numpy.array([self.meanOmega(a, oned=True) for a in apars])
                mO_unp = numpy.array(
                    [
                        super(galpy.df.streampepperdf.streampepperdf, self).meanOmega(
                            a, oned=True
                        )
                        for a in apars
                    ]
        ...
        return signal.csd(x, y, fs=1.0 / (apars[1] - apars[0]), scaling="spectrum")
    ...
    def _sample_aAt(self, n):
        ...
        dangle_at_impact = (
            ...
            - (Om - numpy.tile(self._progenitor_Omega.T, (n, 1)).T) * self._timpact[-1]
        )
        ...
        dOpar = (
            numpy.dot(
                (Om - numpy.tile(self._progenitor_Omega.T, (n, 1)).T).T,
        ...
        for kk, timpact in enumerate(self._timpact[::-1]):
            ...
            dOpar += self._sgapdfs[-kk - 1]._kick_interpdOpar(dangle_par_at_impact)
