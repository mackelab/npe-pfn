...
class HHPosterior(ConditionalDistribution):
    def __init__(self, prior, log_likelihood, device, *args) -> None:
        super().__init__()
        self.prior = prior
        ...
        self.transform = torch.distributions.biject_to(self.prior.support)

        self.transformed_prior = TransformedDistribution(self.prior, self.transform.inv)
        self.potential_fn = lambda x, theta: self.log_likelihood(
            self.transform(theta)
        ).log_prob(x) + self.transformed_prior.log_prob(theta)

    def condition(self, context):
        proposal = SIRDistribution(
            self.transformed_prior, self.potential_fn, context=context, K=32
        )
        k = SliceKernel(self.potential_fn, context)
        mcmc = MCMC(
            k,
            self.potential_fn,
            proposal,
        ...
        return TransformedDistribution(MCMCDistribution(mcmc), self.transform)
...
class HHTask(InferenceTask):
    ...
    def get_true_posterior(self, device: str = "cpu"):
        return HHPosterior(
            self.get_prior_dist(device),
            self.get_loglikelihood_fn(device=device),
            device,
