...
from .utils.distributions import MCMCDistribution, SIRDistribution
...
class SIRTask(InferenceTask):
    def __init__(
        ...
        """
        # This might have problems with SLURM
        base_path = os.path.dirname(os.path.abspath(__file__))
        from diskcache import Cache

        self.cache_params = Cache(os.path.join(base_path, "files/sir_params"))
        self.cache_obs = Cache(os.path.join(base_path, "files/sir_obs"))
        self.cache_posterior = Cache(os.path.join(base_path, "files/sir_posterior"))
        ...
        def likelihood_fn(theta):
            ...
            return torch.distributions.Independent(
                torch.distributions.LogNormal(x.log(), obs_noise),
                1,
            )
    ...
    def get_true_posterior(self, device: str = "cpu"):
        return SIRPosterior(self.get_prior_dist(device), self.get_potential_fn(device))
