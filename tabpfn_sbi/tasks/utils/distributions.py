...
def sample_sir(
    ...
    print(shape, batch_shape)

    proposed_samples = proposal.sample((K,) + shape + batch_shape).squeeze(-2)
    print("Proposed samples", proposed_samples.shape)
    log_potential = potential_fn(proposed_samples)
    log_potential = torch.nan_to_num(
        log_potential, nan=-10000, posinf=10000, neginf=-10000
    )
    logq = proposal.log_prob(proposed_samples)
    logq = torch.nan_to_num(logq, nan=-10000, posinf=10000, neginf=-10000)

    print(log_potential.shape, logq.shape)

    # Now broadcast to match shapes
    log_potential, logq = torch.broadcast_tensors(log_potential, logq)

    log_importance_weights = log_potential - logq
    cum_importance_weights_normalized = torch.softmax(log_importance_weights, 0).cumsum(
        0
    )
    print("log_importance_weights", log_importance_weights.shape)
    ...
    samples = samples.permute(
        *torch.arange(len(shape) - 1, -1, -1), *torch.arange(len(shape), samples.ndim)
    )
    return samples
...
class MixtureDistribution(torch.distributions.Distribution):
    ...
    def __init__(self, mixture_distribution, component_distirbutions) -> None:
        ...
        super().__init__(
            batch_shape=self.component_distirbutions[0].batch_shape,
            event_shape=self.component_distirbutions[0].event_shape,
        )
    ...
    def sample(self, shape=()):
        ...
        return samples.reshape(
            shape
            + self.component_distirbutions[0].batch_shape
            + self.component_distirbutions[0].event_shape
        )
    ...
    def log_prob(self, val):
        ...
        component_log_probs = torch.empty(
            (len(self.component_distirbutions),) + shape, device=val.device
        )
...
from pyro.distributions import MixtureOfDiagNormals  # type: ignore
