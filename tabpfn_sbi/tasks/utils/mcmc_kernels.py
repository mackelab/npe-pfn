...
import torch
from sbi.samplers.vi import get_flow_builder
...
class LangevianKernel(Kernel):
    ...
    @torch.jit.export  # type: ignore
    def init_params(self, x: Tensor):
        self.log_step_size = torch.log(self.step_size) * torch.ones(
...
class LatentSliceKernel(Kernel):
    ...
    def sample(self, x: Tensor) -> Tensor:
        ...
        random_exp = torch.log(1 - torch.rand_like(x, device=self.device)) / (
            -self.step_size
        )
        ...
        for i in range(self.max_resamplings):
            ...
            x_new[mask_reject] = (
                torch.rand_like(a[mask_reject], device=self.device)
                * (b[mask_reject] - a[mask_reject])
                + a[mask_reject]
            )
